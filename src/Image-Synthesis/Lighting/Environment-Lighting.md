...
Let $\displaystyle \operatorname{\mathcal{ZH}}$ be the **ZH (Zonal Harmonics) projection operation**. Analogous to the **Fourier transform**, we have $\displaystyle k_l =\operatorname{\mathcal{ZH}}(\operatorname{f}(\overrightarrow{\omega})) = \int_{\mathrm{S}^2} \operatorname{f}(\overrightarrow{\omega}) \operatorname{\Upsilon_l^0}(\overrightarrow{\omega}) \, d\overrightarrow{\omega}$, and the original function can be reconstructed as the ZH series $\displaystyle\operatorname{f}(\overrightarrow{\omega}) = \sum k_l \operatorname{\Upsilon_l^0}(\overrightarrow{\omega})$.  
