...
The albedo $\displaystyle \operatorname{\rho_{hd}} (\overrightarrow{\omega_o})$ is essentially the norm (zeroth spherical moment) of the BRDF cosine, and we have the normalized BRDF cosine $\displaystyle \frac{\operatorname{f}(\overrightarrow{\omega_i}, \overrightarrow{\omega_o}) \cdot max(0, \cos \theta_i)}{\operatorname{\rho_{hd}} (\overrightarrow{\omega_o})}$.  
...
However, as we state above, the LTC is always normalized. This means that we should use the the norm of the BRDF cosine to scale the LTC first before we approximate the BRDF cosine by the LTC.  
